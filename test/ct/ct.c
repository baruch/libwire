...
static void
runonetest(Test *ts, const char *name)
{
    Test *t;
    for (t=ts; t->f; t++) {
...
readtokens()
{
    int n = 1;
    char c, *s;
	char *make_flags = getenv("MAKEFLAGS");
	if (!make_flags)
		return 1;
    if ((s = strstr(make_flags, " --jobserver-fds="))) {
...
{
	if (argc == 2 && argv[1][0] != '-') {
		printf("Running test %s without forking\n", argv[1]);
		runonetest(ctmaintest, argv[1]);
		return 0;
	}
